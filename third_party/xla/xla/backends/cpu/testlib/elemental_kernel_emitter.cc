...
ElementalKernelEmitter::ElementalKernelEmitter(
    ...
    HloOpcode opcode, ElementalIrEmitterFactory elemental_ir_emitter_factory)
    : input_shapes_(std::move(input_shapes)),
      output_shape_(output_shape),
      opcode_(opcode),
      elemental_ir_emitter_factory_(std::move(elemental_ir_emitter_factory)),
...
ElementalKernelEmitter::EmitKernelSpec() {
  ...
  auto module = std::make_unique<llvm::Module>("elemental_kernel", ctx);

  llvm::IRBuilder<> ir_builder(ctx);

  std::string_view opcode_name = HloOpcodeString(opcode_);
  std::string kernel_name = absl::StrCat("elemental_kernel_", opcode_name);
  llvm::Function* function =
      kernel_api_ir_builder_.EmitKernelFunction(*module, kernel_name);
  ...
  std::unique_ptr<ElementalIrEmitter> elemental_ir_emitter =
      elemental_ir_emitter_factory_(module.get(), &ir_builder);
  ...
  auto source = std::make_unique<LlvmIrKernelSource>(
      context_, std::move(module), kernel_name);
...
std::unique_ptr<ElementalIrEmitter>
ElementalKernelEmitter::ElementalIrEmitterFactoryForTests(
    llvm::Module* module, llvm::IRBuilderBase* builder) {
  return std::make_unique<ElementalIrEmitterForTests>(module, builder);
}
